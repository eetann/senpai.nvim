...
#### 2-1. 設定管理
- [x] A: `/agent_settings` APIエンドポイントを作成（設定の受信と更新）
- [x] B: プラグイン設定（クライアントから）を受け取る処理
- [x] C: `.senpai/agent.json`からプロジェクト設定を読み込む処理
- [x] D: 設定のマージ処理（プロジェクト設定優先）
- [x] E: マージした設定をhonoの変数として保持

#### 2-2. 自動承認判定API
- [x] F: `/tool_auto` APIエンドポイントを作成
- [x] G: ツールタイプごとの自動承認判定ロジック
  - [x] G-1: replace_in_fileの判定（pathを考慮）
  - [x] G-2: execute_commandの判定（commandを考慮）
- [x] H: 設定に基づいて自動承認の可否を返す処理
...
- [x] S: 初回メッセージで`task`じゃなくて`user_feedback`になってるのを修正
