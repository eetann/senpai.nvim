...
function M.render_action_result_border(bufnr, start_row, header_row_length, is_expanded)
...
function M.render_action_result(chat, header, content)
    ...
    M.render_action_result_border(chat.log_area.bufnr, start_row, #header_lines, false)
    ...
    vim.keymap.set('n', '<CR>', function()
        ...
        if current_line >= start_row and current_line <= start_row + #header_lines + 3 then
            M.toggle_action_result_fold(chat.log_area.bufnr, content_start_row + 1, #content_lines)
        end
    end, {
